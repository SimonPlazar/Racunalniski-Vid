...
class KeypointDataset(Dataset):
    ...
    1. On-the-fly generation: generates images during training (pregenerate=False)
    2. Pre-generated: generates all images upfront and stores them in memory (pregenerate=True)

    Args:
        num_samples: Number of samples in the dataset (for synthetic mode)
        image_shape: (H, W) tuple for image dimensions (must be divisible by 8)
        generate_fn: Function that generates (image, keypoints) pairs
                     Should have signature: generate_fn(**generate_kwargs) -> (img, keypoints)
        generate_kwargs: Dictionary of keyword arguments to pass to generate_fn
        transform: Optional transform to apply to images
        augment: Whether to apply data augmentation
        pregenerate: If True, generate all samples upfront and store in memory (faster training)
    """
    ...
    def __init__(
        ...
        # Pre-generate data if requested
    ...
    def _pregenerate_samples(self):
        """Pre-generate all samples and store them in memory."""
        self.pregenerated_data = []
        for i in range(self.num_samples):
            if (i + 1) % 100 == 0:
                print(f"  Generated {i + 1}/{self.num_samples} samples...")

            # Generate image and keypoints
            ...
            self.pregenerated_data.append((img_gray, keypoints.copy()))
    ...
    def __getitem__(self, idx):
        ...
        Returns:
            ...
            target: (65, H/8, W/8) tensor - training target for the network
        """
        # Get data from pre-generated or generate on-the-fly
        if self.pregenerate:
            ...
            # Generate synthetic image and keypoints on-the-fly
            if self.generate_fn is not None:
                img, keypoints = self.generate_fn(**self.generate_kwargs)
            else:
                raise NotImplementedError("Either provide generate_fn")
        ...
        if self.augment:
            img_gray, keypoints = self._augment(img_gray, keypoints)

        # Normalize to [0, 1]
        img_gray = img_gray.astype(np.float32) / 255.0

        # Convert to tensor: (H, W) -> (1, H, W)
        image_tensor = torch.from_numpy(img_gray[np.newaxis, :, :])

        # Prepare training target
        target = prepare_training_target_basic([keypoints], self.image_shape)
